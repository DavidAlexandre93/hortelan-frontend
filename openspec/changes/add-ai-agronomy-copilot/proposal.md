...
- `experience/agronomy-copilot`: User-facing conversational and contextual agronomy assistance, grounded answers, multimodal diagnosis support, safe action proposals, history controls, and trustworthy degraded states.
- `platform/ai-orchestration`: Provider-neutral AI gateway behavior for OpenAI and Gemini routing, structured generation, retrieval, tool execution, safety policy, privacy, idempotency, quotas, observability, and resilience.

### Modified Capabilities

- `architecture/frontend-app-shell`: Add one lazy, authenticated assistant surface that receives only allowlisted route context and remains isolated from public and SSR-critical paths.
...
- `experience/operational-workflows`: Add explain, summarize, diagnose, and draft-action assistance to selected operational workflows while preserving user confirmation and honest data provenance.
- `platform/browser-security`: Extend credential, persistence, consent, and external-resource controls to AI provider access and conversation or image data.
- `delivery/frontend-quality`: Add deterministic AI contract tests, safety and groundedness evaluations, visual and accessibility checks, and latency, cost, and bundle gates.
...
- Frontend: dashboard shell, route context, Hortelan 360, monitoring, alerts, species, reports, help, shared asynchronous states, feature services, Zod DTOs, consent and conversation settings, and responsive end-to-end journeys.
