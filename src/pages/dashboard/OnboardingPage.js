    ...
    structures: [
      {
        ...
        capacity: '180',
        material: 'Madeira',
        sensor: 'sensor_umidade_01',
        actuator: 'valvula_setor_01',
      },
    ],
  },
  {
    ...
    structures: [
      {
        type: 'Vaso',
        name: 'Vaso aromáticas A',
        capacity: '18',
        material: 'Cerâmica',
        sensor: 'sensor_umidade_ervas',
        actuator: 'bomba_irrigacao_01',
      },
      {
        type: 'Vaso',
        name: 'Vaso aromáticas B',
        capacity: '18',
        material: 'Cerâmica',
        sensor: 'sensor_umidade_ervas_02',
        actuator: '',
      },
    ],
  },
...
const speciesTemplates = [
  {
    id: 'alface-crespa',
    label: 'Alface',
    plants: 'alface crespa, cebolinha',
    structure: { type: 'Canteiro', name: 'Canteiro alface', capacity: '120', material: 'Madeira' },
  },
  {
    id: 'manjericao',
    label: 'Manjericão',
    plants: 'manjericão genovês, orégano',
    structure: { type: 'Vaso', name: 'Vaso manjericão', capacity: '15', material: 'Cerâmica' },
  },
  {
    id: 'tomate-cereja',
    label: 'Tomate-cereja',
    plants: 'tomate-cereja, manjericão',
    structure: { type: 'Módulo', name: 'Módulo tomate', capacity: '45', material: 'Plástico' },
  },
];
...
const environmentTemplates = [
  {
    ...
    space: 'estufa',
    sunlight: 'media',
    location: 'Ambiente interno com iluminação complementar',
  },
  {
    id: 'varanda',
    label: 'Varanda',
                  ...
                  <Stack spacing={2}>
                    ...
                    <FormGroup>
                      <FormControlLabel
                        control={
                          <Checkbox
                            checked={checklist.accountCreated}
                            onChange={() => handleChecklistToggle('accountCreated')}
                          />
                        }
                        label="Conta criada"
                      />
                      <FormControlLabel
                        control={
                          <Checkbox
                            checked={checklist.gardenCreated}
                            onChange={() => handleChecklistToggle('gardenCreated')}
                          />
                        }
                        label="Horta criada"
                      />
                      <FormControlLabel
                        control={
                          <Checkbox
                            checked={checklist.plantRegistered}
                            onChange={() => handleChecklistToggle('plantRegistered')}
                          />
                        }
                        label="Planta cadastrada"
                      />
                      <FormControlLabel
                        control={
                          <Checkbox
                            checked={checklist.sensorConnected}
                            onChange={() => handleChecklistToggle('sensorConnected')}
                          />
                        }
                      ...
                      <FormControlLabel
                        control={
                          <Checkbox
                            checked={checklist.notificationsEnabled}
                            onChange={() => handleChecklistToggle('notificationsEnabled')}
                          />
                        }
                      ...
                      <FormControlLabel
                        control={
                          <Checkbox
                            checked={checklist.firstTaskCompleted}
                            onChange={() => handleChecklistToggle('firstTaskCompleted')}
                          />
                        }
                        label="Primeira tarefa concluída"
                      />
                    </FormGroup>
                  </Stack>
                </CardContent>
                    ...
                    <List dense>
                      {guidedFlow.map((item, index) => (
                        <ListItem key={item.id} disablePadding sx={{ mb: 1 }}>
                          <ListItemText primary={`${index + 1}. ${item.title}`} secondary={item.description} />
                          <Chip
                            size="small"
                            color={guideStatus[item.id] ? 'success' : 'default'}
                            label={guideStatus[item.id] ? item.doneLabel : 'Pendente'}
                          />
                        </ListItem>
                      ))}
                    </List>
            ...
            <CardContent sx={{ p: { xs: 2, md: 3 } }}>
              <Stack spacing={2}>
                ...
                >
                  ...
                  <FormControlLabel
                    control={<Checkbox checked={demoMode} onChange={() => setDemoMode((prev) => !prev)} />}
                    label="Ativar demo"
                    sx={{ mr: 0 }}
                  />
                </Stack>
                {demoMode ? (
                  <Alert severity="info" variant="outlined">
                    Dashboard simulado ativo: umidade 41%, luminosidade 78%, reservatório em 63% e recomendação
                    automática de rega para amanhã às 07:00.
                  </Alert>
                ) : (
              ...
              {activeStep === 1 && (
                <Paper variant="outlined" sx={{ p: { xs: 2, md: 2.5 } }}>
                  <Stack spacing={2}>
                    ...
                    <Button
                      startIcon={<AddIcon />}
                      onClick={() => setStructures((prev) => [...prev, createStructure()])}
                      variant="outlined"
                    >
                      Adicionar estrutura
                    </Button>
              ...
              {activeStep === 2 && (
                <Paper variant="outlined" sx={{ p: { xs: 2, md: 2.5 } }}>
                  <Stack spacing={2}>
                    ...
                    <Alert severity="warning" variant="outlined">
                      Alerta pedagógico: evite excesso de rega em ambientes internos; prefira validar umidade antes de
                      cada ciclo.
                    </Alert>
                  </Stack>
                </Paper>
              ...
              {activeStep === 3 && (
                <Paper variant="outlined" sx={{ p: { xs: 2, md: 2.5 } }}>
                  <Stack spacing={2}>
                    ...
                    <Button variant="outlined" onClick={createAutomation} disabled={!automationName.trim()}>
                      Criar automação
                    </Button>
              ...
              {activeStep === 0 && (
                <Paper variant="outlined" sx={{ p: { xs: 2, md: 2.5 } }}>
                  <Stack spacing={2}>
                    ...
                    <Alert severity="info" variant="outlined">
                      Dicas inline aparecem em campos críticos para acelerar sua configuração inicial.
                    </Alert>
                    <Alert severity="info" variant="outlined">
                      Recomendação por etapa: complete um bloco por vez (horta &gt; planta &gt; sensor &gt; automação).
                    </Alert>
                  </Stack>
                </Paper>
              )}
